...
static void
free_rpc_bdev_rbd_wait_for_latest_osdmap_cluster(struct rpc_bdev_rbd_wait_for_latest_osdmap_ctx *req)
{
	free(req->name);
}

static const struct spdk_json_object_decoder rpc_bdev_rbd_wait_for_latest_osdmap_cluster_decoders[] = {
	{"name", offsetof(struct rpc_bdev_rbd_wait_for_latest_osdmap_ctx, name), spdk_json_decode_string, true},
};
...
rpc_bdev_rbd_wait_for_latest_osdmap(struct spdk_jsonrpc_request *request,
				const struct spdk_json_val *params)
{
	struct rpc_bdev_rbd_wait_for_latest_osdmap_ctx req = {NULL};
	int rc;
