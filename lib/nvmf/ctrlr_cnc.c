                             ...
                             uint32_t num_descriptors, uint64_t base_dst_lba, uint32_t *max_block_size)
{
    ...
    *max_block_size = 512;
    ...
    for (uint32_t r = 0; r < num_descriptors; r++) {
        ...
        struct spdk_nvmf_ns *src_ns = spdk_nvmf_subsystem_get_ns(ctx->req->qpair->ctrlr->subsys, rs->src_nsid);
...
cnc_has_overlapping_ranges(struct cnc_context *ctx)
{
    ...
    struct spdk_nvmf_ns *dst_ns = spdk_nvmf_subsystem_get_ns(subsystem, dst_nsid); // spdk_nvmf_subsystem_get_ns(ctx->req->qpair->ctrlr->subsys, rs->src_nsid);
...
issue_next_read_for_range(struct cnc_context *ctx, int r)
{
    if (ctx->draining || ctx->error)
        return 0;
    struct cnc_range_state *rs = &ctx->ranges[r];
    //SPDK_ERRLOG("Issue the next  read  for range ctx %p, remaining %d \n",ctx, rs->remaining);
    if (rs->remaining == 0)
        return 0;
    ...
    for (uint32_t i = 0; i < ctx->config.max_inflight; i++) {
        //int s = atomic_load_explicit(&ctx->chunks[i].state, memory_order_acquire);
    ...
    uint32_t chunk_nlb = (rs->remaining > ctx->config.chunk_nlb) ? ctx->config.chunk_nlb : rs->remaining;
    c->slba = rs->slba;
    c->dlba = rs->dlba;
    c->nlb  = chunk_nlb;
    c->range_index = r;
