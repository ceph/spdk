                        ...
                        struct spdk_nvmf_request *req)
{
    ...
    // -------------------------------------------------------------------------
    if (df != 2) {
        SPDK_ERRLOG("XCOPY Aborted: Format %d unsupported. Requires Format 2h for CNC.\n", df);
        goto reject_invalid_field;
    }

    if (num_descriptors > CNC_MAX_RANGES || dst_nsid == 0) {
        SPDK_ERRLOG("XCOPY Aborted: number descripptors %d , dst ns %d \n", num_descriptors, dst_nsid);
        goto reject_invalid_field;
    }
