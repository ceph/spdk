...
static const struct spdk_json_object_decoder rpc_get_ctrl_io_stats_decoders[] = {
	{"nqn", offsetof(struct rpc_get_ctrl_io_stats_ctx, nqn), spdk_json_decode_string, true},
	{"host_nqn", offsetof(struct rpc_get_ctrl_io_stats_ctx, host_nqn), spdk_json_decode_string, true},
	{"tgt_name", offsetof(struct rpc_get_ctrl_io_stats_ctx, tgt_name), spdk_json_decode_string, true},
	{"reset", offsetof(struct rpc_get_ctrl_io_stats_ctx, reset), spdk_json_decode_bool, true}
};
...
static const struct spdk_json_object_decoder rpc_enable_ctrl_io_stats_decoders[] = {
	{"trtype", offsetof(struct rpc_enable_ctrl_io_stats_ctx, trtype), spdk_json_decode_string, true},
	{"tgt_name", offsetof(struct rpc_enable_ctrl_io_stats_ctx, tgt_name), spdk_json_decode_string, true},
	{"enable", offsetof(struct rpc_enable_ctrl_io_stats_ctx, enable), spdk_json_decode_bool, true}
};
...
static void
nvmf_rpc_visibility_ext_free(struct rpc_nvmf_subsystem_set_ns_visibility_ext *ereq)
{
	struct rpc_nvmf_subsystem_set_ns_visibility_ctx *ctx = ereq->req
	free(ctx->nqn);
	free(ctx->tgt_name);
	free(ereq);
}
			  ...
			  void *cb_arg, int status)
{
	...
	nvmf_rpc_visibility_ext_free(ereq);
		   ...
		   void *cb_arg, int status)
{
	...
	if (spdk_nvmf_subsystem_resume(subsystem, nvmf_rpc_visibility_resumed, ereq)) {
		if (!ereq->response_sent) {
			spdk_jsonrpc_send_error_response(ctx->request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							 "Internal error");
		}
				    ...
				    const struct spdk_json_val *params)
{
	...
	struct rpc_nvmf_subsystem_set_ns_visibility_ctx *ctx;
	struct spdk_nvmf_subsystem *subsystem;
	struct spdk_nvmf_tgt *tgt;
	int rc;
	...
	rc = spdk_nvmf_subsystem_pause(subsystem, ctx->nsid, nvmf_rpc_visibility, ereq);
	if (rc != 0) {
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR, "Internal error");
		nvmf_rpc_visibility_ext_free(ereq);
	}
