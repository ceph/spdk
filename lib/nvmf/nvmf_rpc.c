...
static const struct spdk_json_object_decoder rpc_nvmf_get_ctrl_io_stats_decoders[] = {
	...
	{"verbose_stats", offsetof(struct rpc_nvmf_get_ctrl_io_stats_ctx, verbose_stats), spdk_json_decode_bool, true}
};

/* Bucket structure to hold stats per key */
struct io_stats_bucket {
	char key[256];
	char subnqn[SPDK_NVMF_NQN_MAX_LEN + 1];
	char hostnqn[SPDK_NVMF_NQN_MAX_LEN + 1];
	struct qp_io_stats stats;
	TAILQ_ENTRY(io_stats_bucket) link;
};
TAILQ_HEAD(io_stats_bucket_list, io_stats_bucket);

struct rpc_max_qp_ctx {
	struct io_stats_bucket_list buckets;
	char *req_nqn;
	char *req_host_nqn;
	bool verbose;
...
rpc_collect_qp_stats(struct spdk_io_channel_iter *i)
{
	...
	SPDK_NOTICELOG("rpc_collect_qp_stats invoked for channel %p\n", ch);
	if (!ch) {
	...
	TAILQ_FOREACH(qpair, &pg->qpairs, link) {
		if (!qpair->ctrlr || !qpair->ctrlr->subsys) {
			continue;
		}
		/* Flexible filtering instead of strict ctrlr check */
		if (ctx->req_host_nqn && strcmp(qpair->ctrlr->hostnqn, ctx->req_host_nqn) != 0) {
			continue;
		}
		if (ctx->req_nqn && strcmp(qpair->ctrlr->subsys->subnqn, ctx->req_nqn) != 0) {
			continue;
		}
		...
		if (ctx->reset_stats) {
			...
			/* Key Selection according to 3 rules */
			char key[256] = {0};
			const char *cur_subnqn = qpair->ctrlr->subsys->subnqn;
			const char *cur_hostnqn = qpair->ctrlr->hostnqn;

			if (ctx->req_host_nqn && !ctx->req_nqn) {
				if (ctx->verbose) {
					snprintf(key, sizeof(key), "subsystem: %s", cur_subnqn);
				} else {
					snprintf(key, sizeof(key), "host: %s", ctx->req_host_nqn);
				}
			} else if (ctx->req_nqn && !ctx->req_host_nqn) {
				if (ctx->verbose) {
					snprintf(key, sizeof(key), "host: %s", cur_hostnqn);
				} else {
					snprintf(key, sizeof(key), "subsystem: %s", ctx->req_nqn);
				}
			} else if (!ctx->req_host_nqn && !ctx->req_nqn) {
				if (ctx->verbose) {
					snprintf(key, sizeof(key), "subsystem: %s", cur_subnqn);
				} else {
				...
				snprintf(key, sizeof(key), "host: %s, subsystem: %s", cur_hostnqn, cur_subnqn);
			}

			/* Find or create bucket in ctx->buckets */
			struct io_stats_bucket *b = NULL, *tmp;
			TAILQ_FOREACH(tmp, &ctx->buckets, link) {
			...
			if (!b) {
				...
				if (b) {
					...
					if (ctx->req_host_nqn || (ctx->req_nqn && ctx->verbose)) {
						snprintf(b->hostnqn, sizeof(b->hostnqn), "%s", cur_hostnqn);
					}
					if (cur_subnqn && (ctx->req_nqn || ctx->verbose)) {
						snprintf(b->subnqn, sizeof(b->subnqn), "%s", cur_subnqn);
					}
					spdk_nvmf_stats_reset_qp(&b->stats);
					TAILQ_INSERT_TAIL(&ctx->buckets, b, link);
				} else {
					SPDK_ERRLOG("Failed to allocate memory for I/O stats bucket (key: %s)\n", key);
					spdk_for_each_channel_continue(i, -ENOMEM);
					return;
				}
			}
			if (b) {
				//SPDK_NOTICELOG("Accumulating stats for qp %p \n", qpair);
				spdk_nvmf_accumulate_stats(&b->stats, stats); /* Accumulate into bucket */
				ctx->has_max = true;
			}
		}
	}
...
rpc_collect_done(struct spdk_io_channel_iter *i, int status)
{
	struct rpc_max_qp_ctx *ctx = spdk_io_channel_iter_get_ctx(i);
	struct spdk_json_write_ctx *w;
	struct io_stats_bucket *b, *tmp;

	SPDK_NOTICELOG("rpc_collect_done invoked with status %d, supported=%d, has_max=%d\n",
					status, ctx->supported, ctx->has_max);
	if (status != 0) {
		spdk_jsonrpc_send_error_response(ctx->request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
						"Internal error while collecting statistics");
		goto cleanup;
	}
	w = spdk_jsonrpc_begin_result(ctx->request);
	if (!w) {
		SPDK_ERRLOG("Failed to allocate JSON write context\n");
		spdk_jsonrpc_send_error_response(ctx->request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
						"Failed to allocate JSON write context");
		goto cleanup;
	}
	if (!ctx->supported) {
		...
		/* Loop over buckets to print array instead of single dump */
		spdk_json_write_object_begin(w);
		/* Derive title once for the whole request */
		const char *category = "total";
		if (ctx->req_nqn && ctx->req_host_nqn) {
			category = "controller";
		} else if (ctx->req_host_nqn && !ctx->req_nqn) {
			category = ctx->verbose ? "subsystem" : "host";
		} else if (ctx->req_nqn && !ctx->req_host_nqn) {
			category = ctx->verbose ? "host" : "subsystem";
		} else if (!ctx->req_host_nqn && !ctx->req_nqn && ctx->verbose) {
			category = "subsystem";
		}
		spdk_json_write_named_string(w, "category", category);
		spdk_json_write_named_array_begin(w, "statistics");

		TAILQ_FOREACH(b, &ctx->buckets, link) {
			spdk_json_write_object_begin(w);
			if (b->subnqn[0]) {
				spdk_json_write_named_string(w, "nqn", b->subnqn);
			}
			if (b->hostnqn[0]) {
				spdk_json_write_named_string(w, "host_nqn", b->hostnqn);
			}

			spdk_json_write_name(w, "stats");
			spdk_nvmf_dump_ctrl_stats(w, &b->stats);
			spdk_json_write_object_end(w);
		}
...
cleanup:
	TAILQ_FOREACH_SAFE(b, &ctx->buckets, link, tmp) {
		TAILQ_REMOVE(&ctx->buckets, b, link);
		free(b);
	}
	free(ctx->req_nqn);
	free(ctx->req_host_nqn);
	free(ctx);
}
...
rpc_nvmf_get_ctrl_io_stats(struct spdk_jsonrpc_request *request,
			   const struct spdk_json_val *params)
{
	struct rpc_nvmf_get_ctrl_io_stats_ctx req = {};
	struct spdk_nvmf_tgt *tgt;
	struct rpc_max_qp_ctx *ctx = NULL;

	if (params) {
		if (spdk_json_decode_object(params, rpc_nvmf_get_ctrl_io_stats_decoders,
					SPDK_COUNTOF(rpc_nvmf_get_ctrl_io_stats_decoders),
					&req)) {
			SPDK_ERRLOG("spdk_json_decode_object failed\n");
			spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INVALID_PARAMS, "Invalid parameters");
			return;
	...
	ctx->verbose = req.verbose_stats;
	ctx->supported = true;
	if (req.nqn) {
		ctx->req_nqn = strdup(req.nqn);
		if (!ctx->req_nqn) {
			SPDK_ERRLOG("Memory allocation failed for req_nqn\n");
			spdk_jsonrpc_send_error_response(request,
							SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							"Out of memory");
			free(ctx);
	...
	if (req.host_nqn) {
		ctx->req_host_nqn = strdup(req.host_nqn);
		if (!ctx->req_host_nqn) {
			SPDK_ERRLOG("Memory allocation failed for req_host_nqn\n");
			spdk_jsonrpc_send_error_response(request,
							SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							"Out of memory");
			free(ctx->req_nqn);
	...
	/* subsystem and controller lookup loop not need since filtering happens during channel iteration */
	SPDK_NOTICELOG("RPC starting for_each_channel ctx=%p tgt=%s verbose=%d reset? %d\n",
		       ctx, req.tgt_name, ctx->verbose, ctx->reset_stats);
...
static const struct spdk_json_object_decoder rpc_nvmf_enable_ctrl_io_stats_decoders[] = {
	{"trtype", offsetof(struct rpc_nvmf_enable_ctrl_io_stats_ctx, trtype), spdk_json_decode_string, true},
	{"tgt_name", offsetof(struct rpc_nvmf_enable_ctrl_io_stats_ctx, tgt_name), spdk_json_decode_string, true},
	{"enable", offsetof(struct rpc_nvmf_enable_ctrl_io_stats_ctx, enable), spdk_json_decode_bool, true}
};
...
rpc_nvmf_enable_ctrl_io_stats(struct spdk_jsonrpc_request *request,
			   const struct spdk_json_val *params)
{
	struct rpc_nvmf_enable_ctrl_io_stats_ctx req = {};
	struct spdk_nvmf_tgt *tgt;
	if (params) {
		if (spdk_json_decode_object(params, rpc_nvmf_enable_ctrl_io_stats_decoders,
					SPDK_COUNTOF(rpc_nvmf_enable_ctrl_io_stats_decoders),
					&req)) {
			SPDK_ERRLOG("spdk_json_decode_object failed\n");
			spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INVALID_PARAMS, "Invalid parameters");
			return;
...
cleanup:
	free_rpc_nvmf_enable_ctrl_io_stats(&req);
}
...
static const struct spdk_json_object_decoder rpc_nvmf_create_subsystem_decoders[] = {
	...
	{"max_discard_size_kib", offsetof(struct rpc_subsystem_create, max_discard_size_kib), rpc_decode_max_discard_size_kib, true},
	{"max_write_zeroes_size_kib", offsetof(struct rpc_subsystem_create, max_write_zeroes_size_kib), rpc_decode_max_write_zeroes_size_kib, true},
...
/* TODO: remove in favor of autogenerated decoders */
static const struct spdk_json_object_decoder rpc_nvmf_namespace_decoders[] = {
	{"nsid", offsetof(struct rpc_nvmf_namespace, nsid), spdk_json_decode_uint32, true},
	{"bdev_name", offsetof(struct rpc_nvmf_namespace, bdev_name), spdk_json_decode_string},
	{"ptpl_file", offsetof(struct rpc_nvmf_namespace, ptpl_file), spdk_json_decode_string, true},
	{"nguid", offsetof(struct rpc_nvmf_namespace, nguid), decode_ns_nguid, true},
	{"eui64", offsetof(struct rpc_nvmf_namespace, eui64), decode_ns_eui64, true},
	{"uuid", offsetof(struct rpc_nvmf_namespace, uuid), spdk_json_decode_uuid, true},
	{"anagrpid", offsetof(struct rpc_nvmf_namespace, anagrpid), spdk_json_decode_uint32, true},
	{"no_auto_visible", offsetof(struct rpc_nvmf_namespace, no_auto_visible), spdk_json_decode_bool, true},
	{"hide_metadata", offsetof(struct rpc_nvmf_namespace, hide_metadata), rpc_decode_hide_metadata, true},
};
...
{
	struct rpc_nvmf_namespace *ns_params = out;
...
struct rpc_nvmf_subsystem_set_ns_visibility_ext {
	struct rpc_nvmf_subsystem_set_ns_visibility_ctx req;
	bool response_sent;
};

static const struct spdk_json_object_decoder rpc_nvmf_subsystem_set_ns_visibility_decoders[] = {
	{"nqn", offsetof(struct rpc_nvmf_subsystem_set_ns_visibility_ctx, nqn), spdk_json_decode_string},
	{"nsid", offsetof(struct rpc_nvmf_subsystem_set_ns_visibility_ctx, nsid), spdk_json_decode_uint32},
	{"auto_visible", offsetof(struct rpc_nvmf_subsystem_set_ns_visibility_ctx, auto_visible), spdk_json_decode_bool},
	{"tgt_name", offsetof(struct rpc_nvmf_subsystem_set_ns_visibility_ctx, tgt_name), spdk_json_decode_string, true},
};

static void
free_rpc_nvmf_subsystem_set_ns_visibility_ext(struct rpc_nvmf_subsystem_set_ns_visibility_ext *ereq)
{
	free_rpc_nvmf_subsystem_set_ns_visibility(&ereq->req);
	free(ereq);
}
...
rpc_nvmf_subsystem_set_ns_visibility_resumed(struct spdk_nvmf_subsystem *subsystem,
			  void *cb_arg, int status)
{
	struct rpc_nvmf_subsystem_set_ns_visibility_ext *ereq = cb_arg;

	if (!ereq->response_sent) {
		spdk_jsonrpc_send_bool_response(ereq->req.request, true);
	}

	free_rpc_nvmf_subsystem_set_ns_visibility_ext(ereq);
}
...
{
	struct rpc_nvmf_subsystem_set_ns_visibility_ext *ereq = cb_arg;
	struct rpc_nvmf_subsystem_set_ns_visibility_ctx *req = &ereq->req;
	struct spdk_jsonrpc_request *request = req->request;
	int rc;
	...
	if (spdk_nvmf_subsystem_resume(subsystem, rpc_nvmf_subsystem_set_ns_visibility_resumed, ereq)) {
		if (!ereq->response_sent) {
			spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							 "Internal error");
		}
...
rpc_nvmf_subsystem_set_ns_visibility(struct spdk_jsonrpc_request *request,
				    const struct spdk_json_val *params)
{
	struct rpc_nvmf_subsystem_set_ns_visibility_ext *ereq;
	struct rpc_nvmf_subsystem_set_ns_visibility_ctx *req;
	struct spdk_nvmf_subsystem *subsystem;
	struct spdk_nvmf_tgt *tgt;
	int rc;
	...
	rc = spdk_nvmf_subsystem_pause(subsystem, req->nsid, rpc_nvmf_subsystem_set_ns_visibility_paused, ereq);
	if (rc != 0) {
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR, "Internal error");
		free_rpc_nvmf_subsystem_set_ns_visibility_ext(ereq);
	}
...
static const struct spdk_json_object_decoder rpc_nvmf_subsystem_remove_host_decoders[] = {
	...
	{"keep_connections", offsetof(struct rpc_nvmf_subsystem_remove_host_ctx, keep_connections), spdk_json_decode_bool, true},
...
static const struct spdk_json_object_decoder rpc_nvmf_create_transport_decoders[] = {
	...
	{"io_unit_size", offsetof(struct nvmf_rpc_create_transport_ctx, io_unit_size), rpc_decode_io_unit_size, true},
	{"max_aq_depth", offsetof(struct nvmf_rpc_create_transport_ctx, max_aq_depth), spdk_json_decode_uint32, true},
	{"num_shared_buffers", offsetof(struct nvmf_rpc_create_transport_ctx, num_shared_buffers), rpc_decode_num_shared_buffers, true},
	{"buf_cache_size", offsetof(struct nvmf_rpc_create_transport_ctx, buf_cache_size), rpc_decode_buf_cache_size, true},
...
static const struct spdk_json_object_decoder rpc_nvmf_cnc_set_config_decoders[] = {
	{"host_behav_support_cnc", offsetof(struct rpc_nvmf_cnc_set_config_ctx, host_behav_support_cnc), spdk_json_decode_bool, true},
	{"rate_limit_bytes", offsetof(struct rpc_nvmf_cnc_set_config_ctx, rate_limit_bytes), spdk_json_decode_uint64},
	{"max_inflight", offsetof(struct rpc_nvmf_cnc_set_config_ctx, max_inflight), spdk_json_decode_uint32},
	{"chunk_nlb", offsetof(struct rpc_nvmf_cnc_set_config_ctx, chunk_nlb), spdk_json_decode_uint32},
};

static void
rpc_nvmf_cnc_set_config(struct spdk_jsonrpc_request *request, const struct spdk_json_val *params)
{
	struct rpc_nvmf_cnc_set_config_ctx req = {};

	/* Set default fallbacks to current global values in case user skips an option */

	/* Parse incoming JSON request array parameters */
	if (params && spdk_json_decode_object(params, rpc_nvmf_cnc_set_config_decoders,
					      SPDK_COUNTOF(rpc_nvmf_cnc_set_config_decoders), &req)) {
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INVALID_PARAMS, "Invalid parameters");
		return;
	}
...
struct rpc_set_ana_states_all_call_ctx {
	...
	uint32_t remaining_subsystems;
	uint32_t chunk_subs;   /* max number of subsystems that paused at once */
	uint32_t active_in_chunk;
	uint32_t next_subs_idx;
...
static void rpc_subsystem_paused_cb(struct spdk_nvmf_subsystem *subsystem, void *cb_arg,
				    int status);
...
static const struct spdk_json_object_decoder rpc_nvmf_subsystem_set_ana_states_all_decoders[] = {
	...
	{"chunk_subs", offsetof(struct rpc_nvmf_subsystem_set_ana_states_all_ctx, chunk_subs), spdk_json_decode_uint32, false},
...
static void
dispatch_next_chunk(struct rpc_set_ana_states_all_call_ctx *parent_ctx)
{
	uint32_t start_idx = parent_ctx->next_subs_idx;
	uint32_t end_idx = start_idx + parent_ctx->chunk_subs;

	if (end_idx > parent_ctx->total_subsystems) {
		end_idx = parent_ctx->total_subsystems;
	}

	parent_ctx->active_in_chunk = end_idx - start_idx;
	SPDK_NOTICELOG("Sending chunk of %d elements\n", end_idx - start_idx);
	for (uint32_t i = start_idx; i < end_idx; i++) {
		struct rpc_set_ana_states_all_subsystem_ctx *subs_ctx = &parent_ctx->system_contexts[i];

		spdk_nvmf_subsystem_pause(subs_ctx->subsystem, 0, rpc_subsystem_paused_cb, subs_ctx);
	}
	parent_ctx->next_subs_idx = end_idx;
}
...
rpc_subsystem_resumed_cb(struct spdk_nvmf_subsystem *subsystem, void *cb_arg, int status)
{
	...
	if (status != 0 && parent_ctx->status == 0) {
		...
		subs_ctx->status = status;
	}
	SPDK_NOTICELOG("resumed subsystem %s, remain %d subsystems, active chunk %d\n",
		       subs_ctx->subsystem->subnqn,
		       parent_ctx->remaining_subsystems,
		       parent_ctx->active_in_chunk - 1
		      );
	parent_ctx->active_in_chunk--;
	parent_ctx->remaining_subsystems--;
	if (parent_ctx->active_in_chunk == 0) {
		if (parent_ctx->next_subs_idx < parent_ctx->total_subsystems) {
			/* Process the next batch of 10 subsystems */
			dispatch_next_chunk(parent_ctx);
			return;
		}
	}
	if (parent_ctx->remaining_subsystems == 0) {
		...
		if (parent_ctx->status == 0) {
			...
			struct spdk_json_write_ctx *w = spdk_jsonrpc_begin_result(parent_ctx->request);
			if (w != NULL) {
				spdk_json_write_object_begin(w);
				spdk_json_write_named_bool(w, "success", false);
				spdk_json_write_named_int32(w, "status", parent_ctx->status);

				spdk_json_write_named_array_begin(w, "failed_subsystems");
				for (uint32_t i = 0; i < parent_ctx->total_subsystems; i++) {
					struct rpc_set_ana_states_all_subsystem_ctx *s = &parent_ctx->system_contexts[i];
					if (s->status != 0) {
						spdk_json_write_object_begin(w);
						spdk_json_write_named_string(w, "nqn", spdk_nvmf_subsystem_get_nqn(s->subsystem));
						spdk_json_write_named_int32(w, "status", s->status);
						spdk_json_write_object_end(w);
					}
				}
				spdk_json_write_array_end(w);
				spdk_json_write_object_end(w);
				spdk_jsonrpc_end_result(parent_ctx->request, w);
			}
		}
...
rpc_subsystem_paused_cb(struct spdk_nvmf_subsystem *subsystem, void *cb_arg, int status)
{
	...
	if (listener_count == 0) {
		SPDK_NOTICELOG("paused subsystem no listeners - to resume %s\n", subsystem->subnqn);
		spdk_nvmf_subsystem_resume(subsystem, rpc_subsystem_resumed_cb, subs_ctx);
		return;
	}
				      ...
				      const struct spdk_json_val *params)
{
	...
	ctx->system_contexts = calloc(total_subsystems,
				      sizeof(struct rpc_set_ana_states_all_subsystem_ctx));
	if (!ctx->system_contexts) {
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR, "Out of memory");
		goto cleanup;
	}
	/* Total subsystems to context */
	ctx->total_subsystems = total_subsystems;
	if (req.chunk_subs == 0 || req.chunk_subs > total_subsystems) {
		req.chunk_subs = total_subsystems;
	}
	ctx->chunk_subs = req.chunk_subs;
	ctx->total_remaining = total_calls; /* total calls per subsystem-listener-anagrp */
	uint32_t subs_idx = 0;
	...
	SPDK_NOTICELOG("Number ana states changes to do totally %d, num subsystems %d chunk size %d\n",
		       ctx->total_remaining, ctx->remaining_subsystems, ctx->chunk_subs);
	if (ctx->remaining_subsystems == 0) {
		spdk_jsonrpc_send_bool_response(request, true);
		goto cleanup;
	}
	dispatch_next_chunk(ctx);
...
SPDK_RPC_REGISTER("nvmf_subsystem_set_ana_states_all", rpc_nvmf_subsystem_set_ana_states_all,
		  SPDK_RPC_RUNTIME);
