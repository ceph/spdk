...
static const struct spdk_json_object_decoder nvmf_rpc_subsystem_anagrpid_decoder[] = {
	{"nqn", offsetof(struct nvmf_rpc_anagrpid_ctx, nqn), spdk_json_decode_string},
	{"nsid", offsetof(struct nvmf_rpc_anagrpid_ctx, nsid), spdk_json_decode_uint32},
	{"anagrpid", offsetof(struct nvmf_rpc_anagrpid_ctx, anagrpid), spdk_json_decode_uint32},
	{"tgt_name", offsetof(struct nvmf_rpc_anagrpid_ctx, tgt_name), spdk_json_decode_string, true},
};

static void
nvmf_rpc_anagrpid_ctx_free(struct nvmf_rpc_anagrpid_ctx *ctx)
{
	free(ctx->nqn);
	free(ctx->tgt_name);
	free(ctx);
...
nvmf_rpc_anagrpid_resumed(struct spdk_nvmf_subsystem *subsystem,
			  void *cb_arg, int status)
{
	struct nvmf_rpc_anagrpid_ctx *ctx = cb_arg;
	struct spdk_jsonrpc_request *request = ctx->request;
	bool response_sent = ctx->response_sent;

	nvmf_rpc_anagrpid_ctx_free(ctx);
...
static void
nvmf_rpc_anagrpid(struct spdk_nvmf_subsystem *subsystem,
		  void *cb_arg, int status)
{
	struct nvmf_rpc_anagrpid_ctx *ctx = cb_arg;
	int rc;

	rc = spdk_nvmf_subsystem_set_ns_anagrpid(subsystem, ctx->nsid, ctx->anagrpid);
	if (rc != 0) {
...
static void
rpc_nvmf_subsystem_set_ns_anagrpid(struct spdk_jsonrpc_request *request,
				   const struct spdk_json_val *params)
{
	struct nvmf_rpc_anagrpid_ctx *ctx;
	struct spdk_nvmf_subsystem *subsystem;
	struct spdk_nvmf_tgt *tgt;
	int rc;
	...
	if (spdk_json_decode_object_relaxed(params, nvmf_rpc_subsystem_anagrpid_decoder,
					    SPDK_COUNTOF(nvmf_rpc_subsystem_anagrpid_decoder), ctx)) {
		SPDK_ERRLOG("spdk_json_decode_object failed\n");
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INVALID_PARAMS, "Invalid parameters");
		nvmf_rpc_anagrpid_ctx_free(ctx);
		return;
	...
	rc = spdk_nvmf_subsystem_pause(subsystem, ctx->nsid, nvmf_rpc_anagrpid, ctx);
	if (rc != 0) {
		if (rc == -EBUSY) {
			spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							 "subsystem busy, retry later.\n");
...
SPDK_RPC_REGISTER("nvmf_subsystem_set_ns_anagrpid", rpc_nvmf_subsystem_set_ns_anagrpid,
		  SPDK_RPC_RUNTIME)
