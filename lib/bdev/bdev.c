			    ...
			    struct spdk_bdev_ext_io_opts *opts)
{
	...
	return bdev_writev_blocks_with_md(desc, ch, iov, iovcnt, md, offset_blocks, num_blocks,
					  ...
					  false, nvme_cdw12_raw, nvme_cdw13_raw, crc32c, has_crc32c, cb, cb_arg);
}
...
static inline int
bdev_nvme_passthru_check_io_type(struct spdk_bdev *bdev, enum spdk_bdev_io_type io_type,
				 void *md_buf)
{
	switch (io_type) {
			    ...
			    spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	...
	rc = bdev_nvme_passthru_check_io_type(bdev, io_type, md_buf);
	if (spdk_unlikely(rc != 0)) {
		return rc;
	}
				...
				struct spdk_bdev_ext_io_opts *ext_opts)
{
	...
	if (ext_opts) {
		if (SPDK_GET_FIELD(ext_opts, dif_check_flags_exclude_mask, 0) != 0 ||
			...
			SPDK_ERRLOG("Only memory domain, its context, accel_sequence, metadata, and md_len are supported\n");
			return -EINVAL;
		}
